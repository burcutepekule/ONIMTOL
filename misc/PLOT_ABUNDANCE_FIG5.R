...
combined_data_norm = combined_data_norm %>% filter(type=='Model')
# Plotting
...
if(plot1 == 1){
  ...
  p_rel = ggplot(combined_data_norm, aes(x = days, y = value, color = taxa)) +
    geom_line(size = 0.8) +  
    theme_minimal() +
...
if(blob_on==1){
  p_rel = p_rel + 
    annotate("point", x = 20, y = 0.74, shape = 21, fill = '#f20026', color = "#61000f", size = 5*max(lumen_daily_cuc_df_m1$value), stroke = 1) +
    annotate("text", x = 20, y = 0.765, label = paste0(round(max(lumen_daily_cuc_df_m1$value),2)), hjust = -0.1, vjust = -1, size = 4, color = "black") 
}
...
seq_ticks = seq(0, 0.25, 0.05)
if(max(lumen_daily_cuc_df$median)>0.25){
...
p_abs=ggplot(lumen_daily_cuc_df, aes(x = days, y = median, color = taxa, linetype = coating)) +
  geom_line(size = 0.8) +  theme_minimal() +
  labs(title = "Absolute abundances in gut lumen", x = "DOL", y = "(Cells/g Feces) x 1e-11") +
  scale_color_manual(values = my_colors) +
...
if(blob_on==1){
  p_abs = p_abs + 
    annotate("point", x = 20, y = y_up-0.05/2, shape = 21, fill = '#f20026', color = "#61000f", size = 70*max(lumen_daily_cuc_df_m1$median), stroke = 1) +
    annotate("text", x = 20, y = y_up-0.05/2+0.005, label = paste0(round(max(lumen_daily_cuc_df_m1$median),3)), hjust = -0.1, vjust = -1, size = 4, color = "black") 
}

# p_abs = p_abs + 
#   annotate("point", x = 30, y = 0.12, shape = 21, fill = '#f20026', color = "#61000f", size = 500*lumen_daily_cuc_df_d14$median, stroke = 1) +
#   annotate("text", x = 30, y = 0.125, label = "0.074", hjust = -0.1, vjust = -1, size = 4, color = "black") +
#   theme(plot.margin = margin(1, 4, 1, 1, "cm")) # Adjust margins to ensure the blob and text fit
...
y_out_df_use_micro = y_out_df_use
...
p_micro <- ggplot(df_long, aes(x = t, y = value, color = series, linetype = series)) +
  ...
  theme(legend.title = element_blank(),
        ...
        legend.box.just = "right", # Increased bottom margin for padding
...
p_pcr=ggplot(df_lumen_tot_agg_wide_m1, aes(x = days, y = pcr), color='gray10')+
  geom_line(size = 0.8) +  theme_minimal() +
  labs(title = "Enterobacteriaceae ratio, first month", x = "DOL", y = "") +
  theme(legend.title = element_blank(), plot.title = element_text(size = 12)) 
